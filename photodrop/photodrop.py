...
class PhotoDrop(commands.Cog):
    ...
    async def pp_calendar(self, ctx: commands.Context, user: discord.Member, month: int | None = None, year: int | None = None):
        ...
        with a photo collage of the month so far.
        """
        ...
        entries = []
        collage_entries: list[tuple[int, str, Path | None]] = []
        for day_num in range(1, days_in_month + 1):
            ...
            if status is None:
                continue
            photo_path = None
            if status in (STATUS_FULL, STATUS_TARDY):
                raw_paths = storage.list_photos(self.data_dir, user.id, date_key)
                if raw_paths:
                    photo_path = raw_paths[0]
            collage_entries.append((day_num, status, photo_path))

        month_label = f"{calendar_module.month_name[month]} {year}"
        embed = embeds.calendar_embed(user.display_name, month_label, entries)

        if not collage_entries:
            await ctx.send(embed=embed)
            return
    ...
    async def _post_poll(self, channel: discord.TextChannel, question: str, entries: list[tuple[str, str, Path]], kind: str) -> None:
        ...
        poll = discord.Poll(question=question, duration=timedelta(hours=DEFAULT_POLL_DURATION_HOURS))
        option_labels = {}
